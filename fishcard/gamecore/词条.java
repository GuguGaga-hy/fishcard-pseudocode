...
public class 词条 {
    ...
    private static void 施加负面效果(玩家 目标, 负面效果类型 类型, int 层数, 卡牌 来源) {
        ...
        switch (类型) {
            case 中毒:
                新层数 = 目标.中毒计数 + 层数;
                目标.中毒计数 = Math.min(10, Math.max(0, 新层数));
                break;
            case 诅咒:
                新层数 = 目标.诅咒计数 + 层数;
                目标.诅咒计数 = Math.min(10, Math.max(0, 新层数));
                break;
            case 魅惑:
                新层数 = 目标.魅惑计数 + 层数;
                目标.魅惑计数 = Math.min(10, Math.max(0, 新层数));
                break;
            case 狂暴:
                新层数 = 目标.狂暴计数 + 层数;
                目标.狂暴计数 = Math.min(10, Math.max(0, 新层数));
                break;
            case 冰冻:
                新层数 = 目标.冰冻计数 + 层数;
                目标.冰冻计数 = Math.min(10, Math.max(0, 新层数));
                break;
        }
    }
